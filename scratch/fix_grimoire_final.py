...
match_pattern = r'      <div className="absolute top-0 left-0 w-full flex justify-between z-20 pointer-events-none p-2">.*?      </div>\n\n      \{\/\*.*?\}\n      <div className="flex-1 flex items-center justify-center w-full min-h-0 py-16">'

content = re.sub(match_pattern, right_stack + '\n\n      <div className="flex-1 flex items-center justify-center w-full min-h-0 py-16">', content, flags=re.DOTALL)

with open('src/components/game/Grimoire.tsx', 'w', encoding='utf-8') as f:
    f.write(content)
print("Fixed Grimoire layout")

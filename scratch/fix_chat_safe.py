...
text = re.sub(
    r'export const Chat = \(\{ roomId, userUid, userName, isHost, players, hostPlayer, isEvil, settings, seatCount = 15 \}: ChatProps\) => \{',
...
# 2. Update states
state_old = """  const [activeChannel, setActiveChannel] = useState<string>('town_square');
  const [messages, setMessages] = useState<Message[]>([]);
  const [inputText, setInputText] = useState('');
  const [isDropdownOpen, setIsDropdownOpen] = useState(false);
  const messagesEndRef = useRef<HTMLDivElement>(null);"""

state_new = """  const [activeChannel, setActiveChannel] = useState<string>('town_square');
...
# 4. Insert Unread Computing before the main return JSX
unread_logic = """
...
text = text.replace('  return (\n    <div className="flex flex-col h-full bg-black/40">', unread_logic)

# 5. Fix badge inside dropdown items
item_old = """                  <button
                    key={ch.id}
                    onClick={() => {
                      if (!ch.disabled) {
...
text = text.replace(item_old, item_new)

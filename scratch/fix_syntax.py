...
grimoire_stack = """{/* 右側浮動資訊柱 */}
      <div className="absolute right-4 top-16 bottom-4 flex flex-col space-y-4 items-end pointer-events-none z-20 overflow-y-auto no-scrollbar pb-6 w-64 pr-2">
              ...
              return (
                <div key={i} className="flex flex-col items-center flex-1">
                  <div 
                    ...
                    className="w-12 h-12 sm:w-14 sm:h-14 rounded-full border-2 border-white/60 bg-black/80 flex flex-col items-center justify-center shadow-lg relative overflow-hidden mb-1 cursor-pointer group"
                  >
                    {role ? (
                      <>
        ...
        {/* 傳奇角色 */}
        <div 
          className="bg-black/60 border border-yellow-500/30 rounded-xl p-3 shadow-lg backdrop-blur-md flex flex-col pointer-events-auto w-full transform scale-105 origin-right cursor-pointer hover:border-yellow-500/60 transition-colors"
...
if stack_start != -1 and stack_end != -1:
    content = content[:stack_start] + grimoire_stack + '\n' + content[stack_end:]
...
stack_start2 = content2.find('{/* 右側浮動資訊柱 */}')
stack_end2 = content2.find('      <div className="flex-1 flex items-center justify-center w-full min-h-0 py-16">')

center_stack = """{/* 右側浮動資訊柱 */}
      <div className="absolute right-4 top-16 bottom-4 flex flex-col space-y-4 items-end pointer-events-none z-20 overflow-y-auto no-scrollbar pb-6 w-64 pr-2">
          ...
          <div className="flex space-x-2 justify-center w-full">
            {[0, 1, 2].map(i => {
              ...
              return (
                <div key={i} className="flex flex-col items-center flex-1">
                  <div 
                    className="w-12 h-12 sm:w-14 sm:h-14 rounded-full border-2 border-white/60 bg-black/80 flex flex-col items-center justify-center shadow-lg relative overflow-hidden mb-1"
                  >
                    {canSeeBluffs ? (
                      role ? (
                        <RoleIcon icon={role.icon} className="w-full h-full object-cover bg-[radial-gradient(circle_at_center,_#f4e5c5_0%,_#dcb37b_100%)]" />
        ...
        {/* 傳奇角色 */}
        <div 
          className="bg-black/60 border border-yellow-500/30 rounded-xl p-3 shadow-lg backdrop-blur-md flex flex-col pointer-events-auto w-full transform scale-105 origin-right"
        >
...
"""
if stack_start2 != -1 and stack_end2 != -1:
    content2 = content2[:stack_start2] + center_stack + '\n' + content2[stack_end2:]

...
export const RoleSelectionModal = ({ isOpen, onClose, onSelect, script, filterType = 'normal', selectedFabled = [], noOverlay = true }: RoleSelectionModalProps) => {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} maxWidth="max-w-[80vw]" noOverlay={noOverlay} title={filterType === 'fabled' ? "選擇傳奇角色" : <div className="flex items-center gap-[30px] w-full min-w-[300px] md:min-w-[400px]"><span>選擇角色</span><div className="flex gap-2 tracking-normal"><button onClick={() => setActiveTab("script")} className={`px-4 py-1 text-base font-bold rounded-lg border-2 transition-colors ${activeTab === "script" ? "bg-indigo-600/60 border-indigo-500 text-white shadow-[0_0_10px_rgba(99,102,241,0.5)]" : "bg-slate-800/60 border-slate-600 text-slate-400 hover:text-white"}`}>劇本</button><button onClick={() => setActiveTab("traveler")} className={`px-4 py-1 text-base font-bold rounded-lg border-2 transition-colors ${activeTab === "traveler" ? "bg-indigo-600/60 border-indigo-500 text-white shadow-[0_0_10px_rgba(99,102,241,0.5)]" : "bg-slate-800/60 border-slate-600 text-slate-400 hover:text-white"}`}>旅行者</button></div></div>}>
